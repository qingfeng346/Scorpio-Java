...
public class DefaultScriptUserdataObject extends ScriptUserdata {
    ...
    public DefaultScriptUserdataObject(Script script, Object value) {
        super(script);
        this.setValue(value);
        this.setValueType((value instanceof Class<?>) ? (Class<?>)getValue() : value.getClass());
        m_Type = script.GetScorpioType(getValueType());
    }
    @Override
    public ScriptObject Call(ScriptObject[] parameters) throws Exception {
        return getScript().CreateObject(m_Type.CreateInstance(parameters));
    }
    @Override

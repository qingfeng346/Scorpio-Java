...
public class DefaultScriptUserdataObject extends ScriptUserdata
{
	...
	public DefaultScriptUserdataObject(Script script, Object value)
	{
		...
		this.setValue(value);
		this.setValueType((value instanceof Class<?>) ? (Class<?>)getValue() : getValue().getClass());
		...
		for (int i = 0; i < methods.length;++i)
		{
			...
			{
				m_Functions.put(name, m_Script.CreateFunction(new ScorpioMethod(getValueType(), name, getValue())));
			}
		}
	}
	private Field GetField(String strName) throws Exception
	{
	...
	@Override
	public ScriptObject Call(ScriptObject[] parameters) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, InstantiationException, Exception
	{
	...
	public ScriptObject GetValue(String strName) throws Exception
	{
		...
		if (field != null)
		{
			return m_Script.CreateObject(field.GetValue(getValue()));
		}
